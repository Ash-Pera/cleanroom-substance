...
if sys.byteorder != "little":
    raise RuntimeError("this parser assumes a little-endian host")
...
def u32_view(d):
    """The whole buffer as a u32 array. Scanning this is far cheaper than calling
    struct.unpack_from once per candidate offset - the footer search alone made
    millions of such calls, and it dominated parse time."""
    return memoryview(d)[: len(d) & ~3].cast("I")


def _footer_ok(d, a, off, n, cmax):
    """Does the 16 bytes at `off` satisfy the footer's arithmetic signature?"""
    if off < 0x38 or off + 16 > n or off & 3:
        return None
    i = off >> 2
    c = a[i]
    if c == 0 or c > cmax:
...
def find_footer(d, a=None):
    ...
    It is also **the last sixteen bytes of the file** - 435 of 435 specimens, no
    exceptions - so the whole-file scan this used to do was answering a question the
    layout already settles. The scan is kept as a fallback so that a file which does
    not follow the convention still parses; it simply never runs in practice.
    """
    n = len(d)
    if a is None:
        a = u32_view(d)
    cmax = n // 4
    tail = _footer_ok(d, a, n - 16, n, cmax)
    if tail is not None:
        return [tail]
    hits = []
    for i in range(0x38 // 4, len(a) - 3):
        c = a[i]
        if c == 0 or c > cmax:
            continue
        r0 = a[i + 1]
